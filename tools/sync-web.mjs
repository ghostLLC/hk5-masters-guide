...
const FILES = ['index.html', 'data.js', 'app.js', 'store.js', 'track.js'];
...
if (!/id="trackBody"/.test(html)) problems.push('缺少跟进表容器 #trackBody');
if (!/id="syncBar"/.test(html)) problems.push('缺少同步状态条 #syncBar');
for (const bad of ['SUPABASE_ANON_KEY', 'SUPABASE_URL', 'QODER_PAT', 'DATABASE_URL', '.scrape/', 'dev/track-preview']) {
  for (const f of FILES) {
...
console.log('\n自检通过：5 个文件齐备、脚本引用完整、无平台密钥与内部路径。');
